#!/usr/bin/env python3
"""Check chapters 17–18 with original setup."""
...
def main() -> None:
    result_file = ROOT / '.generated/stage4c-source-checks.json'
    result_file.unlink(missing_ok=True)
    data, actual = catalog.validate()
    ids = [m['block'] for m in data['compiled_mappings']
           if m['block'][:4] in {'ch17', 'ch18'} or m['block'] in RELATED]
    expected = [b['id'] for b in data['blocks'] if b['id'][:4] in {'ch17', 'ch18'}] + RELATED
    if len(ids) != len(set(ids)) or sorted(ids) != sorted(expected):
        raise ValueError('Stage 4-C coverage differs from all chapter blocks and related examples')
    ...
    directory = ROOT / '.generated/stage4c/source-checks'

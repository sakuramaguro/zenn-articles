...
Lean では距離空間を **型クラス（型の免許証）** として実装しています。第1巻で学んだ「型クラス ＝ 免許証」の比喩を思い出してください。`MetricSpace X` は距離関数とその法則、対応する位相などをまとめた構造です。`[MetricSpace X]` でその構造を前提として受け取り、標準的な型では登録済みのインスタンスを利用できます。

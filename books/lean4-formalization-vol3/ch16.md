...
| | `lintegral` | `integral`（本章の実数値の場合） |
|---|---|---|
| 関数の型 | `α → ℝ≥0∞` | `α → ℝ` |
| 積分の値の型 | `ℝ≥0∞`（∞を含む） | `ℝ` |
| 定義の範囲 | 可測でない関数にも定義されます | 可積分でない関数にも定義され、その場合は0です |
| 通常の積分として使うとき | 定理に応じて可測性などを確認します | 可積分性を確認します |

`integral`は実数値だけでなく、適切な型クラスのもとでベクトル値のBochner積分も扱います。
...
非負の実数値関数については、非負性とa.e.強可測性を確認し、`integral_eq_lintegral_of_nonneg_ae`で`integral`と`lintegral`の`.toReal`を結び付けます。非可積分の場合の規約値にも注意してください。符号のある関数やベクトル値の関数について、無条件に同じ式を使うことはできません。
...
[第18章の総合演習](ch18)では、`volume.restrict (Set.Icc 0 1)` を明示し、関数とその二乗の可積分性を示してから積分値を計算します。同じ式でも、測度を実数全体の `volume` に変えると可積分性の結論が変わるため、積分記号だけでなく測度まで確認してください。

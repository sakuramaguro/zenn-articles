...
`open 名前空間 in` は、続く式・宣言・タクティクの範囲で、名前を探す対象の名前空間を追加します。効果を確かめるため、自作の定理を使います。
...
二つの `#check` は、どちらも `BookOpenDemo.add_swap` の型を表示します。短い名前で参照できる範囲は `open ... in` の直後だけです。この例では、`open BookOpenDemo in` を外した位置で `#check add_swap` と書くと、名前を解決できません。完全な名前なら、その範囲の外でも参照できます。

:::message alert
**⚠️ `open` で短縮されない名前と、同名の定理**

`protected` として宣言された名前は、その名前空間を `open` するだけでは短縮できません。固定環境の `Nat.add_comm` はその例です。`#print Nat.add_comm` で `protected theorem` と表示されることを確認できます。
...
短い `add_comm` は、`Nat.add_comm` の `Nat.` を省いたものではなく、root（名前空間の接頭辞がない場所）にある一般の定理です。自然数にも適用できるので、コードが成功するだけでは、どの名前を参照したか判断できません。`#check` の出力で名前と型を確認してください。

同じ短い名前が複数の名前空間にある場合も、完全な名前を使うと参照先を明確にできます。`import Mathlib` はモジュールの読み込み、`open` は名前を探す範囲の指定です。Mathlibの宣言はrootや `Nat`・`Real`・`MeasureTheory` などに分かれており、`open Mathlib` で全定理を短縮できるわけではありません。
:::
...
| 名前空間を省略して書く | `open 名前空間 in`（protectedな名前を除く） | 4.4節の `open BookOpenDemo in add_swap a b` |
...
- 引数は2個（`n m : ℕ`）
- `#check` は名前と型、`#print` は宣言と処理後の証明項まで表示する

ここでの `…` は本書による省略で、Leanに入力するコードではありません。`#print` の証明項は、元のソースに書かれたタクティク列と同じ表示になるとは限りません。`protected` の意味は4.4節で確認したとおりです。

...
Part 2の `Filter.Tendsto` は、像フィルターと収束先のフィルターの順序関係で収束を記述しました。一方、a.e.の命題は `μ.ae` に関する `Filter.Eventually` です。両方で同じフィルターの枠組みを使っています。`ae_restrict_le` が `(μ.restrict s).ae ≤ μ.ae` を与え、そこから `ae_restrict_of_ae` が従います。この方向には `s` の可測性は必要ありません。
:::
...
`Measurable.aemeasurable` で可測性から AE 可測性を得て、`AEMeasurable.congr` で a.e. 等しい関数へ転送します。[第18章の総合演習](ch18)でもa.e.等式を使います。可測性は積分を扱う準備、可積分性は有限な期待値を扱う条件、a.e.等式は代表関数を変えても積分が変わらないことに対応します。これらは別々の性質です。
:::

...
$\mathbb{R}$・$\mathbb{R}^n$・$\mathbb{C}$ には `ProperSpace` のインスタンスがあります。第5章の $\ell^2$ では単位閉球が非コンパクトであることを証明し、`ProperSpace` が成り立たないと結論します。**`inferInstance` の失敗だけから、この否定は導けません。** 探索失敗には、インポートや仮定、登録されたインスタンスの不足という可能性もあります。
:::
...
| Heine-Borel（同値形） | 閉 ∧ 有界 ↔ コンパクト | `Metric.isCompact_iff_isClosed_bounded` |

...
第7章では、距離から開球や近傍を扱う方法を学びました。本章では、距離を指定せずに開集合と連続性を扱うための構造を学びます。
...
第7章では「開集合とは各点の周りに開球が入る集合」と定義しました。しかしこの定義は距離関数 `dist` に依存しています。
...
- **弱位相**：連続線形汎関数の値を通じて収束を調べる位相。第10章で扱うノルム位相とは区別します。
...
距離空間からは擬距離空間の構造が得られ、擬距離空間からは位相が得られます。次の図は、構造の一部を忘れて使う向きを表すものとして読んでください。

![距離空間から擬距離空間、位相空間へ構造を忘れる関係の模式図](/images/vol2/fig-08.png)
...
この仕組みにより、距離空間でも位相空間についての定理を使えます。ただし、第7章で距離の性質を使った証明まで、任意の位相空間へ一般化されるわけではありません。
:::
...
第6章を思い出してください。あのとき著者は `nhds a` を「点 $a$ の近傍フィルター」と紹介し、距離空間では `Metric.ball a ε` の族が基底になると説明しました。

ここでは、近傍の記述を一般の位相空間で確認します。

> 「`nhds a` そのものはどう**定義**されているのか？」

第7章の `Metric.nhds_basis_ball` は、近傍を開球で調べるための特徴付けでした。`nhds` 自体は、開集合を使って距離に依存せず定義できます。
...
第7章まで `nhds a` は「距離 `dist` を通じて開球で定義される」ものだと思っていたかもしれません。しかし `nhds_def` を見ると、`dist` は一切登場しません。**`IsOpen` という概念さえあれば `nhds` が定義できる**のです。
...
第6章で著者が `nhds` を「望遠鏡のレンズ」と呼んだのは、「$a$ を含む開集合たちを通して見る方向」を意味していました。その比喩は今、`⨅ s ∈ {s | a ∈ s ∧ IsOpen s}, 𝓟 s` という精密な定義として回収されました。
:::
...
距離を指定していない位相空間では、開球を使うことはできません。それでも `nhds_def` や開近傍による基底を使えます。第10章の $\ell^2$ はノルムから得られる距離・位相で扱います。
:::

---

## 8.4 合成定理の証明 ──── InfoView完全実況
...
$\mathbb{R}$・$\mathbb{R}^n$・$\mathbb{C}$ には `ProperSpace` のインスタンスがあります。第10章の $\ell^2$ では単位閉球が非コンパクトであることを証明し、`ProperSpace` が成り立たないと結論します。**`inferInstance` の失敗だけから、この否定は導けません。** 探索失敗には、インポートや仮定、登録されたインスタンスの不足という可能性もあります。
:::
...
答えは**完備性（completeness）**です。「すべての Cauchy 列が収束する」という性質が、無限次元解析の生命線です。次章ではこの `CompleteSpace` 型クラスと、Cauchy 列の形式化に踏み込みます。そこで初めて、第11章のラスボス「Banachの不動点定理」の前提が揃います。
:::
...
特に `Cauchy`（Cauchy フィルター）と `CompleteSpace`（完備空間）の定義を、第7章の `Filter.Tendsto` と同じ「フィルターの言語」で統一的に理解します。
:::
...
この等式を確認し、$\{e_n\}$ から収束部分列が取れないことを直感的に説明してください。（Leanでのコードは第10章で扱います。）

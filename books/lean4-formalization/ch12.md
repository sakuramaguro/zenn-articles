...
### Part 2の到達点 ──── 完備性という魔法の鍵

Part 2では、フィルターの言語から出発し、距離空間・位相空間・完備距離空間という階段を一段ずつ上り、最終章で**Banachの不動点定理**に辿り着きました。

Part 2では、[第6章](ch06)の収束から、第7・8章の距離と位相、第9・10章の完備性と無限次元空間を経て、[第11章](ch11)でBanachの不動点定理を適用しました。これは学習順序であり、型クラスの継承関係ではありません。
...
Part 2の最終節では、この定理を使って常微分方程式の解の一意存在が示せる予告も置きました。完備性は「Cauchy列の収束先が空間内に存在すること」を保証する道具でした。
...
Part 2を書き終えた勢いでPart 3の執筆に入り、「次は $L^2$ 空間を形式化しよう」と Mathlib を眺め始めました。`Lp` という型を見つけたのはよかったのですが、`Lp ℝ 2 μ` の `μ` が何者か調べると `Measure α` という初見の型が出てきます。`Measure α` を理解しようとすると今度は `MeasurableSpace` が登場し……気づいたときには「積分を定義するためには測度論を一から積み上げる必要がある」という壁に直面していました。完備性さえあれば積分できるわけではなかったのです。
:::
...
では Lean のコードで言うと、最終的にはどこに辿り着くのでしょうか。Part 3の到達点を先に見ておきましょう。
...
`∫⁻`、`∀ᵐ`、`liminf`——見慣れない記号が並んでいます。これらを一つずつ解きほぐしていくのがPart 3のミッションです。

---

### Part 3の地図 ──── これから登る山の全景
...
| **[第18章](ch18)** | $L^p$ 空間と確率空間 | Part 4（予定）への橋 | `Lp`, `IsProbabilityMeasure` |
...
`IsProbabilityMeasure` は「$P(\Omega) = 1$ を満たす測度」を意味します。上の `#check` は積分式の型を確認しているだけです。`Integrable X P` の仮定は、有限な実数値の期待値として扱うために用意しています。これが確率空間 $(\Omega, \mathcal{F}, P)$ の Lean での表現です。Part 4では、この設定を出発点としてfiltration・martingale・Itô積分を扱う予定です。これらの章は現在の収録範囲には含まれていません。そのとき、Part 3で積み上げた測度論の道具立てがすべて活きてきます。
:::
...
`[MeasurableSpace X]` という角括弧記法は、「$X$ には可測空間の構造が入っている」という**型クラスの免許証**です。Part 1・Part 2で登場した `[MetricSpace X]` や `[NormedAddCommGroup E]` と同じ仕組みです。この構造があると `MeasurableSet`・`Measurable` を記述できます。具体的な集合や関数の可測性は、それぞれ証明する必要があります。
:::

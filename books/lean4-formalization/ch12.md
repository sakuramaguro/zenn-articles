...
![Part 2の章構成の積み上げ図（Filter.Tendsto → MetricSpace → TopologicalSpace → CompleteSpace → ContractingWith）を示す概念図](/images/vol3/fig-01.png)
...
![Part 3の章構成の積み上げ図（σ-代数 → 測度 → 可測関数 → 単関数 → lintegral/integral → 収束定理 → Lp/確率空間）を示すピラミッド型の概念図](/images/vol3/fig-02.png)
...
| **第13章** | 測度空間の建設 | σ-代数に「大きさ」を割り当てる | `Measure α`, `volume`, `IsProbabilityMeasure` |
| **第14章** | 可測関数と AE の哲学 | 「積分できる関数」の候補を選別 | `Measurable`, `AEMeasurable`, `∀ᵐ` |
| **第15章** | 単関数と積分の足場 | 積分の建設現場 | `SimpleFunc`, `SimpleFunc.lintegral` |
| **第16章** | Lebesgue 積分の正体 | 積分の完成 | `∫⁻`（lintegral）, `∫`（integral）, `Integrable` |
| **第17章** | 収束定理・三部作 | 極限と積分の交換 | `lintegral_iSup`, `lintegral_liminf_le`, `tendsto_integral_of_dominated_convergence` |
| **第18章** | $L^p$ 空間と確率空間 | Part 4（予定）への橋 | `Lp`, `IsProbabilityMeasure` |

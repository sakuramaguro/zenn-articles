...
第5章末尾では、連続性の仮定がないと証明できない予告コードを置きました。まず、そのコードに `hf : ContinuousAt f a` を加えた修正版を見ます。
...
第5章末尾の予告を、意図したエラー例として再掲します。これは連続性の仮定がない形です。完成例とは別ファイルで実行してください。
...
![6.1節、予告コードでexact?が沈黙しているVS Codeのスクショ](/images/vol2/fig-01.png)

:::message alert
**⚠️ これは著者（teru）が実際に体験した出来事です**

第1部を書き終えた勢いで第2部の執筆に入り、自分で書いた予告コードを証明しようとしたところ、`exact?`、`apply?`、`simp?`、`aesop` をすべて試しても証明できませんでした。気づくまでに約40分かかりました。
:::
...
つまり著者は第1部の末尾に、**証明できないコードを「予告」として堂々と置いていたわけです。**
...
![6.3節、フィルターを望遠鏡のレンズに例えた概念図](/images/vol2/fig-02.png)
...
この「完備束として設計し、`NeBot` で真のフィルターを選別する」という分離が Mathlib の美しい設計です。第9章で Cauchy フィルターを扱う際に `NeBot` が再登場します。
:::
...
![6.4節、点aの周りの複数の同心円（開集合）をnhdsという箱でまとめている図。第7章のMetric.ballへの強烈な伏線](/images/vol2/fig-03.png)
...
第1部で学んだ「型クラス ＝ 免許証」の比喩で言えば、`nhds` は「`TopologicalSpace` の免許を持つ型であれば、その点の近傍フィルターを取り出せます」という宣言です。
:::
...
![6.6節、exact hf で No goals になった歓喜のVS Codeスクショ](/images/vol2/fig-04.png)
...
表の最後の行にある `nhdsWithin a (Set.Ioi a)` という記法は少し複雑に見えるかもしれません。「右側（$a$ より大きい側）から近づく」方向を表すフィルターなのですが、この記法の詳細は第8章で改めて扱います。今は「方向を指定してフィルターを絞り込めるんだな」程度の認識で大丈夫です。
:::
...
> **著者より**：第1部の予告コードが偽の命題だったと気づいたとき、正直「やってしまった」と思いました。しかし Claude とのやり取りを40分ほど続けているうちに、その「失敗」こそが `ContinuousAt` の定義を調べるきっかけになり、`Iff.rfl` で証明が通った瞬間の驚きにつながりました。本章の体験談と画面画像は執筆時の記録であり、掲載コードと本文の出力は本書の固定版に合わせてあります。

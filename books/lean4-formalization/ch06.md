...
[第5章](ch05)末尾では、連続性の仮定がないと証明できない予告コードを置きました。まず、そのコードに `hf : ContinuousAt f a` を加えた修正版を見ます。
...
[第5章](ch05)末尾の予告を、意図したエラー例として再掲します。これは連続性の仮定がない形です。完成例とは別ファイルで実行してください。
...
Part 1を書き終えた勢いでPart 2の執筆に入り、自分で書いた予告コードを証明しようとしたところ、`exact?`、`apply?`、`simp?`、`aesop` をすべて試しても証明できませんでした。気づくまでに約40分かかりました。
:::
...
つまり著者はPart 1の末尾に、**証明できないコードを「予告」として堂々と置いていたわけです。**
...
**ここまでの確認（6.3節）**：`S : Set ℕ` に対して、`S ∈ Filter.atTop` と `n ∈ S` は何が違いますか。

:::details 答えを確認する
`n ∈ S` は、一つの自然数が集合に属するという意味です。`S ∈ Filter.atTop` は、ある `N` 以上のすべての自然数を `S` が含むという意味で、`∃ N : ℕ, ∀ n ≥ N, n ∈ S` と表せます。次の節では、点の周囲にある集合をまとめた近傍フィルターへ進みます。
:::
...
Part 1で学んだ「型クラス ＝ 免許証」の比喩で言えば、`nhds` は「`TopologicalSpace` の免許を持つ型であれば、その点の近傍フィルターを取り出せます」という宣言です。
:::
...
**ここまでの確認（6.5節）**：`Tendsto f (nhds a) (nhds b)` の二つのフィルターは、それぞれ何を指定していますか。

:::details 答えを確認する
`nhds a` は入力が点 `a` に近づくこと、`nhds b` は出力が点 `b` に近づくことを指定します。実数の場合は、出力に求める近さ `ε` に対し、入力に十分な近さ `δ` を選べることと対応します。ここでの `nhds a` は点 `a` 自身を除いていません。次の節では `b = f a` として連続性との対応を確かめます。
:::
...
ここまでの道具を使って、Part 1の予告コードに戻りましょう。
...
表の最後の行にある `nhdsWithin a (Set.Ioi a)` は、右側（$a$ より大きい側）から近づくことを表します。本章末の問3「別解を見る」では、穴あき近傍を使う具体例と、右極限・左極限の記法を確認します。今は、集合を指定して近づき方を制限できることを押さえてください。
:::
...
**ここまでの確認（6.8節）**：`hf : ContinuousAt f a` と `hg : ContinuousAt g (f a)` から合成の連続性を示すとき、なぜ `hg.comp hf` と書くのでしょうか。

:::details 答えを確認する
合成 `g ∘ f` は、先に `f`、次に `g` を適用します。`hf` は入力点 `a` での `f` の連続性、`hg` はその出力点 `f a` での `g` の連続性です。外側の関数についての `hg` に、内側の関数についての `hf` を渡すと、`ContinuousAt (g ∘ f) a` が得られます。
:::
...
最後に、Part 1の予告コードを3つの正しい形に書き直します。
...
[第5章](ch05)5.7節の「呪文」は、次のような構造をしていました。
...
> **著者より**：Part 1の予告コードが偽の命題だったと気づいたとき、正直「やってしまった」と思いました。しかし Claude とのやり取りを40分ほど続けているうちに、その「失敗」こそが `ContinuousAt` の定義を調べるきっかけになり、`Iff.rfl` で証明が通った瞬間の驚きにつながりました。本章の体験談は執筆時の記録であり、掲載コードと本文の出力は本書の固定版に合わせてあります。

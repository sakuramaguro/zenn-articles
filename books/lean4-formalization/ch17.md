...
支配条件 $\lVert F_n(x)\rVert\leq1$ は $n=0$ を別に処理します。連続性、有限測度上の定数関数の可積分性、支配条件、収束の4つが揃ったところで、優収束定理を適用します。最後の `simpa` は極限関数0の積分を0へ書き換えます。完成例を読み返すときは、`hFm` がAE強可測性、`integrable_const 1` が優関数の可積分性、`sineSequence_norm_le_one` が支配、`sineSequence_tendsto` が収束に対応することを確認してください。定数1が可積分なのは、ここでは有限な制限測度を使っているためです。実数全体のLebesgue測度では定数1は可積分ではありません。
:::

...
第12章の可測空間、第13章の測度、第14章の可測関数とa.e.等式、第15・16章の積分を使います。本章では既存の定理の仮定を揃え、具体例へ適用します。
:::
...
<details>
<summary>✅ 解答の続き：各点収束と真の不等式</summary>
...
第12章の可測空間から、積分と極限の交換まで進みました。`lintegral_iSup`・`lintegral_liminf_le`・`tendsto_integral_of_dominated_convergence` の仮定と結論を、使いたい関数列の性質に対応させて確認してください。

...
**問題 6.2**　Fatouの補題で等号が成立しない反例（$f_n = \mathbf{1}_{[n, n+1)}$）を Lean で形式化してください。まず各項の積分を求め、次に各点収束と両辺の極限下限を確認します。
...
**問題 6.3**　$n\geq1$ で $F_n(x) = \sin(nx)/n$ とし、$F_0(x)=0$ と定めます。`tendsto_integral_of_dominated_convergence` を使って、$[0,1]$ 上の積分が0に収束することを示してください。優関数 $g\equiv1$ が使えます。Leanでは0による除算を0と定義しているため、下の式は $n=0$ でもこの定義と一致します。
...
まず $\lVert F_n(x)\rVert\leq1/n$ を示し、はさみうちで各点収束を得ています。支配条件 $\lVert F_n(x)\rVert\leq1$ は $n=0$ を別に処理します。連続性、有限測度上の定数関数の可積分性、支配条件、収束の4つが揃ったところで、優収束定理を適用します。最後の `simpa` は極限関数0の積分を0へ書き換えます。
...
解答の定義と評価補題を引き継ぐ別解です。`norm_integral_le_of_norm_le_const` により積分のノルムを $1/n$ と区間の測度の積で押さえます。$[0,1]$ の測度が1なので、最後にはさみうちを使えます。
...
→ `lintegral_iSup` では `intro m n hmn a` と進め、`f m a ≤ f n a` を示します。これは各点での単調性で、AE 命題へ変換する必要はありません。AE 可測性・AE 単調性だけを持つ場合は `lintegral_iSup'` の型を確認します。

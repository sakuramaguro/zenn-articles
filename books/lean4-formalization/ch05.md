...
```text
-- All Messages パネルの表示
declaration uses `sorry`   ← 未証明部分を含む宣言への警告
-- エラー：0件
```

![sorry骨格の黄色波線警告](https://storage.googleapis.com/zenn-user-upload/dd2c3500c20e-20260324.png)

通常の設定では、この骨格は `sorry` の警告付きで受理されます。ただし、これは3つの課題が証明可能だという保証ではありません。確認できたのは、未証明部分を仮置きしたコードの型の整合性です。紙の上の議論やMathlibの定理と照合しながら、それぞれの課題を実際の証明で置き換えていきます。警告をエラーとして扱う設定では、`sorry` が残る間はビルドに失敗します。
...
`have` は局所的な事実に名前を付け、後続の証明で使えるようにします。その証明を `sorry` で仮置きすると、まだ証明していない事実も使えてしまいます。**型検査が通っても、その事実が仮定から導けるとは限りません。** `have` の型と証明方針を確認し、仮置きを解消する必要があります。
...
最後に、同じファイルの定理宣言の後で公理依存を確認します。骨格版と完成版の `dimension_theorem` は、同時に並べず置き換えてください。
...
この完成例には `sorryAx` への依存がありません。`No goals` はその位置のゴールがなくなったことを示し、公理の一覧は定理が何に依存するかを示します。エラー・警告に加えてこの一覧を確認し、定義・仮定・結論が意図した次元定理と一致するかも読み直します。

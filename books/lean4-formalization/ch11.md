...
その位置の未解決ゴールがなくなったときの表示：

```text
-- No goals ✓
```

`No goals` は、カーソル位置に未解決のゴールがないことを示します。`sorry` でもゴールを閉じられるため、この表示だけでは不動点定理の適用が検証済みかは分かりません。完成を確認するときはファイル全体のエラー・警告、主要定理の `#print axioms`、仮定と結論を確認します。上のコードのように採用版のMathlibとAPIが一致しない箇所は、修正して実行を確認する必要があります。
:::
...
| 第5章 | `lp`・単位閉球の非コンパクト性 | 無限次元でもコンパクトでなくても定理が使える |
...
- **位相空間の骨格**（第3章）：`TopologicalSpace`・`IsCompact`・`ProperSpace`
- **完備性の免許証**（第4章）：`CompleteSpace`・`CauchySeq`
- **無限次元の洗礼**（第5章）：$\ell^2$ 空間と Heine-Borel の崩壊
- **不動点定理の勝利**（第6章）：`ContractingWith.existsFixedPoint`

Leanが検査するのは、形式化した命題の証明が、使用する定義・仮定・公理の下で成立しているかです。完成した主要定理では `#print axioms` で `sorryAx` や追加した公理への依存を調べます。標準的な古典公理への依存は、それだけで欠陥を意味しません。また、型検査は、書いた定義や仮定が意図した数学の問題に対応しているかを自動では判定しません。
...
example : ∃! z : ℝ, (fun x => x / 3 + 1) z = z := by
  ...
  -- ステップ③：一意性（hf の縮小性から）
  exact huniq y hy
  -- 要修正：不動点APIの呼び出しを確認する。完成した解答としては未検証
```

...
本章で学ぶ「3点セット質問」は、第4章・第5章でMathlibの定理や必要な仮定を調べるときにも使えます。第5章では、核と像の有限次元性を確認し、既存の次元定理を使って証明を完成させます。
:::
...
第5章でも、型クラスの宣言、現在のゴール、数学的意図をそろえて候補を調べます。AIが提案した名前は `#check` で確認し、実際の証明をLeanで検査してください。
:::
...
名前の誤記、必要なimportや名前空間の不足、使用しているMathlibの版の違い、存在しないAPIの提案などが原因になります。Claudeが返してきた定理名を使ってエラーが出た場合は、後者を疑ってください。
...
第5章5.4節では、`exact?` で核と像の有限次元性を調べ、明示的な補題や `infer_instance` で書く方法と比べます。5.5節では次元等式の補題を `#check` で確認して使います。
:::
...
「`failed to synthesize instance HAdd α α`」というエラーは、数学の言葉に翻訳すると「**現在の環境では必要な加法のインスタンスが見つかりません**」という意味です。集合 $\alpha$ に加法の演算を与える前に `a + b` と書こうとしたのと同じことです。型クラスの免許証を追加するという作業は、数学で「以下では $\alpha$ を加法群とする」と前提を書くことに対応しています。この対応に慣れると、型クラスエラーが「どの代数構造が欠けているのか」を正確に告げてくれる道標になります。
:::

:::message
**🔗 第5章への伏線④**

第5章5.3節では、型クラスの前提を付けずに線形写像の型を書いたときのエラーを確認します。その後、体・加法可換群・加群の仮定をそろえた宣言を組み立てます。個々のエラー数ではなく、各仮定が何を表すかに注目してください。
:::
...
第5章5.2節では、核・像の有限次元性と次元等式を `sorry` で仮置きします。前二つは仮定を理解するための確認課題です。完成例では既存の次元等式の補題を直接使い、`sorryAx` への依存がなくなったことを確認します。
:::
...
次の第4章では、多数の定義・定理を収録するMathlibを調べる技術を学びます。「どこに目的の定理があるか分からない」という不安が、`#check` と `exact?` と検索の技術によって解消されていく過程を一緒に見ていきます。

...
ここで $\sim$ は「$\mu$-a.e. 等しい」という同値関係です。商をとるのは、「a.e. 等しい関数は同じもの」と見なすためです。$p=2$ の場合、商をとる前は異なる関数の間でも擬距離が0になり得ますが、商をとった $L^2$ では距離0なら同じ元です。Mathlibの `Lp ℝ 2 μ` は、この商をとった後のノルム空間・距離空間に対応します。ノルムは

...
![測度の直観図。σ-代数 M の各可測集合（区間・複雑な集合）に非負の実数または正の無限大（大きさ）を割り当てる矢印を示す。μ: M → [0,+∞] というマッピングのイメージ](/images/vol3/fig-04.png)

第12章で「どの集合が測れるか」を決める σ-代数を作りました。次は「測れる集合に具体的な大きさを割り当てる」関数を作ります。これが**測度**です。
...
`volume` がとる値の型は `ℝ≥0∞`（= `ENNReal`、拡張非負実数）です。`ENNReal.ofReal r` は通常の実数 $r \geq 0$ を `ℝ≥0∞` に埋め込む関数で、$r < 0$ の場合は $0$ を返します。Lebesgue 測度が $[0, +\infty]$ に値をとるため、この型が必要になります。`a > b` なら区間は空集合であり、`ofReal (b-a) = 0` と整合します。非負積分の値域としても第16章で使います。
:::
...
`Real.volume_Icc` が測度の値を `ENNReal.ofReal (b - a)` で表すのを初めて見たとき、「なぜ普通の実数じゃないのか」と戸惑いました。第16章で `lintegral` の値域が `ℝ≥0∞` であることを学ぶと、この設計の一貫性が見えてきます。測度論の型設計は深いです。

...
第5章では、既存の次元定理の仮定と結論を調べ、目的の等式へ適用します。本章ではその準備として、名前を探し、`#check` で型を確かめ、必要に応じてソースを読む流れを学びます。探索やAIの提案が得られたら、手元の固定版で実際に検証しましょう。
...
本章で学ぶ探索ツール4本（`#check`・`#print`・`exact?`・`apply?`）と外部ツール（Mathlib4Docs・Loogle）は、必要な補題を調べる助けになります。第5章では核・像の有限次元性と次元等式の型を確認し、完成証明に使います。
:::
...
JavaScriptの開発環境と比べるなら、**Lake** が依存管理とビルドの役割を担います。`lakefile.toml` が `package.json` に対応し、依存関係は `lake-manifest.json` に記録されます。`lake build` はビルド、`lake exe cache get` はMathlibのビルド済みファイルの取得です。`npm install` と同じ処理だと考えないでください。ただし Mathlib のビルドキャッシュは数GBあり、初回ダウンロードに時間がかかる点は第1章で経験済みです。
:::
...
第5章5.5節では `LinearMap.finrank_range_add_finrank_ker` の型を `#check` で確認します。検索やAIから得た名前も、仮定・結論を確認してから使います。
:::
...
→ 提案が得られたら、型と証明を確認して使う。第5章5.4節では核と像の有限次元性を調べます。
...
→ 第5章では、既存の等式と目的の等式で両辺の向きと加算の順番が違うことを確認します。
...
多くの定理が検証された状態で収録されているということは、数学の一大部分が「証明済みの事実」として積み重なっているということです。その巨大な武器庫の前に立ち、一本の定理を手に取る——その体験を、次の第5章で一緒にしましょう。

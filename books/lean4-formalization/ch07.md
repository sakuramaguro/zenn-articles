...
第6章の終わりで、著者は「`nhds a` の基底が `Metric.ball a δ`（$\delta$-開球）だ」と言いました。その章では近傍との対応を確認しましたが、距離空間の構造自体はまだ詳しく扱っていません。`Metric.ball` とは何か。`dist` とは何か。`MetricSpace` とは何か。
...
Lean では距離空間を**型クラス（型の免許証）**として実装しています。第1部で学んだ「型クラス ＝ 免許証」の比喩を思い出してください。`MetricSpace X` は距離関数とその法則、対応する位相などをまとめた構造です。`[MetricSpace X]` でその構造を前提として受け取り、標準的な型では登録済みのインスタンスを利用できます。
...
例えば二乗可積分な可測関数を点ごとの等号で区別すると、異なる関数でもほとんど至る所等しければ $L^2$ の擬距離は0になります。そこで、距離0の関数を同一視した商を取ると、通常の距離空間 $L^2$ が得られます。商を取る前と後を区別しましょう。第10章の数列空間 $\ell^2$ も、通常の距離空間です。
:::
...
第6章で予告した `Metric.ball` を、いよいよ正面から見ていきます。
...
![ユークリッド空間の開球と閉球を、距離が半径に等しい点の有無で対比した図](/images/vol2/fig-06.png)
...
第6章で「`nhds a` の基底は `Metric.ball a ε` だ」と言いました。この接続を今こそ正確に理解しましょう。
...
## 7.5 本章の勝利命題①：開球は開集合 ──── InfoView完全実況
...
第6章では `Metric.tendsto_nhds_nhds` という定理を「ある」と言いつつ使いました。今度はこの定理が**なぜ成り立つか**を、距離空間の構造から理解します。
...
第10章では**無限次元空間** $\ell^2$（絶対値の二乗が総和可能な数列の全体）を扱います。本章の開球や連続性の定理は引き続き使えますが、有限次元の場合との違いに注意します。
...
第8章・第9章で `CompleteSpace` と完備化を学び、第10章で `ℓ²` の構築を実況中継します。これらの章で、距離・位相・完備性・コンパクト性の役割を区別していきます。
:::
...
> **著者より**：距離の3公理を Lean に打ち込んだ瞬間、「数学の授業で黒板に書いた公理が、型チェッカーにそのまま認識される」という体験に軽く感動しました。数学は仕様書であり、Lean はそのコンパイラです。第8章では、この「距離」をさらに抽象化した「位相」を見ていきます。
